...
export function processEvent(data) {
  ...
  if (eventType === 'round_start') {
    // Dynamically expand pipeline if this round exceeds current count
    const roundNum = data.round || 1;
    const stateOrder = window._dynamicStateOrder || STATE_ORDER;
    const clarifyingIdx = stateOrder.indexOf('CLARIFYING');
    const roundSteps = stateOrder.filter(s => s.startsWith('ROUND')).length;
    if (roundNum > roundSteps && clarifyingIdx > 0) {
      const container = document.getElementById('phaseIndicator');
      if (container) {
        const clarifyingEl = container.querySelector('[data-phase=CLARIFYING]');
        const arrow = document.createElement('span');
        arrow.className = 'phase-arrow';
        arrow.textContent = '\u2192';
        const step = document.createElement('span');
        step.className = 'phase-step';
        step.dataset.phase = 'ROUND' + roundNum;
        step.textContent = '\uD83D\uDD0D Round ' + roundNum;
        if (clarifyingEl) {
          const ref = clarifyingEl.previousElementSibling;
          if (ref && ref.classList.contains('phase-arrow')) {
            container.insertBefore(step, ref);
            container.insertBefore(arrow, ref);
          } else {
            container.insertBefore(arrow, clarifyingEl);
            container.insertBefore(step, clarifyingEl);
          }
          window._dynamicStateOrder = [
            ...stateOrder.slice(0, clarifyingIdx),
            'ROUND' + roundNum,
            ...stateOrder.slice(clarifyingIdx),
          ];
        }
      }
    }
